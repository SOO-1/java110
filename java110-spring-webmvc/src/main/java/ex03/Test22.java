...
// RestController로 지정하였으나, json이 아니라 text/plain으로 출력하고 싶을 경우 ,
// produces로 설정하면 된다.
//
@RestController // => @Controller + @ ResponseBody   모든 값들은 json으로 출력
@RequestMapping("/ex03/test22")
public class Test22 {
    
    // 페이지 컨트롤러에 @RestController를 붙이면,
    ...
    //
    // => MappingJackson2HttpMessageConverter 는 다음 라이브러리에 들어있다.
    //
    ...
    // serialize
    // produces="text/plain;charset=UTF-8" 가 value뒤에 쓰여있었기 때문에 Error 발생!
    // 프론트컨트롤러가 text가 plain이라고 지정했기 때문! 
    // json 데이터이기 때문에 없애야 함!
    ...
    // @RestController 일 경우, 리턴 타입이 String 이라면 그대로 출력한다.
    @RequestMapping("m4") 
    ...
    // => 생략하면,
    //    클라이언트가 보낸 데이터가 일반 application/x-www-form-urlencoded 형식이라고
    //    간주한다.
