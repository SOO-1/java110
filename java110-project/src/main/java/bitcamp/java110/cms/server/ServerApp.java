...
// 톰켓 서버가 이 서블릿을 실행하도록 등록한다.
// 예) http://localhost:8888/java110-project/*
@WebServlet("/app/*")   // 여기 나온 부분까지가 servletpath, *부분에 해당되는게 pathinfo
public class ServerApp implements Servlet{
    ...
    public void init(ServletConfig config) throws ServletException {
        // 서블릿 컨테이너는 (java application 관리자)
        // 이 서블릿 객체를 생성한 후 
    ...
    public void service(
        ...
        // 이 메서드에서 요청을 처리 할 컨트롤러의 메서드를 찾아 호출하면 된다.

        // 예) http://localhost:8888/manager/list
        // => HTTP 프로토콜 정보를 다루려면 request 객체를 원래의 타입으로 
        //    캐스팅 한 후 사용하라!

        HttpServletRequest httpRequest = (HttpServletRequest)request;   //request의 원래타입은 httpservletrequest!
...
//        System.out.println(servletPath);
        RequestMappingHandler mapping = 
                requestHandlerMap.getMapping(pathInfo.substring(1)); //물음표 앞의 값

        response.setContentType("text/plain;charset=utf-8");    //getWriter얻기 전에!
