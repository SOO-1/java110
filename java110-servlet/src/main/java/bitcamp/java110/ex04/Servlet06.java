...
public class Servlet06 extends GenericServlet {
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
         ...
         *       => 그런데 톰캣 7 이하에서는 server.xml에 설정을 해줘야 한다.
         *          다음과 같이 Connector 태그에  URIEncoding 속성을  추가하라. (server.xml)
...
/*        
        ...
        req.setCharacterEncoding("UTF-8");  //들어오는데이터 안깨지게 하기
*/
        String name = req.getParameter("name");
        res.setContentType("text/plain;charset=UTF-8"); //나가는 데이터 안 깨지게 하기. getwriter 호출 전!
 ...
 *      "ASCII 문자화 된 데이터를 원래의 바이트로 변환하는 것" = "URL 디코딩"
 * => 웹브라우저는 데이터를 보내기 전에 URL 인코딩한다.
 *    원래 ASCII 문자는 그대로 두고 한글과 같은 7비트를 초과하는 코드인 경우,
 *    URL 인코딩을 진행한다.
 * => 웹 서버는 웹브라우저로 부터 받은 URL 인코딩 값을 디코딩하여
 ...
 *    개발자가 java.net.URLEncoder를 사용하여 직접 URL 인코딩 해야 한다.
 * => 또한 웹브라우저가 보낸 데이터를 웹서버를 통해 받지 않고, 직접 받는 경우가 있다.
 *    이런 경우에도 개발자가 java.net.URLDecoder를 사용하여
 *    직접 소켓 프로그래밍으로 URL 디코딩을 해야한다.
 */
