...
// 서블릿을 만들었으면 서블릿 컨테이너에 등록해야 한다. => 그 때 사용하는 것이 @WebServlet()
...
public class Servlet01 implements Servlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        // 이 메서드에서는 서블릿이 작업할 때 사용할 자원을 준비하는 코드를 둔다.
        // 예를 들면, DB 커넥션을 준비한다거나, 외부 서버에 소켓을 연결하는 등의 작업이다.
        // 또는, 서블릿에서 사용 할 서비스 객체나 DAO 객체를 준비하는 일을 한다.
    ...
    public String getServletInfo() {
        // 서블릿을 실행 중에 직접 호출 할 일은 없다.
        // 주로 서블릿 컨테이너에 관리 메뉴에서 서블릿 정보를 출력할 때
        // 이 메서드에 리턴 값을 받아서 출력한다.
        return "Servlet01";
    }
...
/* [서블릿 구동 절차]
 * - 클라이언트가 요청한 URL에 응답 할 서블릿을 찾는다.
 * - 해당 서블릿의 인스턴스가 이미 생성되었다면,
 *      - service()를 호출한다.
 * - 해당 서블릿 인스턴스가 아직 생성되지 않았다면,
 *      - 서블릿 객체 생성 및 생성자 호출
 *      - init() 호출
 *      - service() 호출
